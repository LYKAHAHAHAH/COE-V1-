...
import { searchConfigFile, createConfigFile, updateConfigFile, readConfigFile, uploadImageToDrive } from './services/driveStore';
...
export default function App() {
  ...
  // Load centralized configuration on mount + Auth sync setup
  React.useEffect(() => {
    // Check local server configuration fallback first
    async function loadCentralAssets() {
      try {
        ...
        console.error("Error loading local centralized assets configuration:", err);
      }
    }
    loadCentralAssets();

    // Subscribe to Google Firebase OAuth sign-in flow
    const unsubscribe = initAuth(
      async (firebaseUser, token) => {
        ...
        try {
          ...
          const fileId = await searchConfigFile(token);
          if (fileId) {
            setDriveFileId(fileId);
            const centralAssets = await readConfigFile(token, fileId);
            if (centralAssets) {
              setAssets(centralAssets);
            }
          ...
          console.error("Failed to search and parse Google Drive config file:", err);
          setSyncStatus('error');
        }
      },
      () => {
  ...
  // Save state to centralized configuration (debounced to prevent spamming during drag/resize)
  React.useEffect(() => {
    ...
    const handler = setTimeout(async () => {
      if (accessToken) {
        try {
          setSyncStatus('syncing');
          if (driveFileId) {
            await updateConfigFile(accessToken, driveFileId, assets);
          } else {
            const newFileId = await createConfigFile(accessToken, assets);
            setDriveFileId(newFileId);
          }
          setSyncStatus('synced');
        } catch (err) {
          console.error("Failed to auto-sync state to Google Drive:", err);
          setSyncStatus('error');
        }
      } else {
  ...
  }, [assets, accessToken, driveFileId]);
  ...
  const handleLogoUpload = (key: keyof CertificateAssets['logos']) => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        ...
        // Prefer Google Drive upload if accessToken is available
        if (accessToken) {
          try {
            setSyncStatus('syncing');
            const url = await uploadImageToDrive(file, accessToken);
            updateLogo(key, { src: url });
            setSyncStatus('synced');
            return;
          } catch (err) {
            console.error("Google Drive upload failed for logo, trying server fallback:", err);
          }
        }

        // Fallback to server local/S3 upload
        try {
      ...
      reader.onloadend = async () => {
        ...
        if (accessToken) {
          try {
            ...
            console.error("Google Drive upload failed for signature, trying server fallback:", err);
          }
        }
      ...
      <div className="no-print">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
            ...
            <div className="flex items-center gap-2.5 bg-zinc-900 border border-zinc-800 rounded-lg p-1">
              {isAuthLoading ? (
                ...
                <div className="flex items-center gap-3 pl-2.5 pr-1 py-1">
                  <div className="flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                    <span className="text-[11px] font-bold text-zinc-300 max-w-[120px] truncate" title={user.email || ""}>
                      {user.displayName || (user.email ? user.email.split('@')[0] : "Active")}
                    </span>
                  </div>
                  
                  <span className="text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider bg-zinc-850 text-blue-400 border border-blue-500/10">
                    {syncStatus === 'syncing' ? 'Syncing...' : syncStatus === 'synced' ? 'Synced' : syncStatus === 'error' ? 'Error' : 'Drive'}
                  </span>
                  ...
                  <button
                    ...
                    className="text-[10px] font-bold text-zinc-500 hover:text-red-400 transition-colors uppercase px-2 py-1 hover:bg-zinc-800/60 rounded cursor-pointer"
                  >
                    Logout
                  </button>
                </div>
                ...
                <button
                  onClick={async () => {
                    try {
                      ...
                      if (result) {
                        ...
                        setSyncStatus('syncing');
                        const fileId = await searchConfigFile(result.accessToken);
                        if (fileId) {
                          setDriveFileId(fileId);
                          const centralAssets = await readConfigFile(result.accessToken, fileId);
                          if (centralAssets) {
                            setAssets(centralAssets);
                          }
                          setSyncStatus('synced');
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 bg-zinc-800 hover:bg-zinc-750 hover:text-white transition-all text-xs font-bold text-zinc-300 uppercase tracking-wider rounded-md cursor-pointer border border-zinc-700/50"
                >
