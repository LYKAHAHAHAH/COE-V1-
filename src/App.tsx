...
export default function App() {
  ...
  // Keep track of the last known state shared with the server to prevent feedback loops
  const lastServerSyncedAssetsRef = useRef<string>('');

  const persistAssetsToServer = async (newAssets: CertificateAssets) => {
    try {
      const currentStr = JSON.stringify(newAssets);
      lastServerSyncedAssetsRef.current = currentStr;
      ...
      console.error("Failed to persist assets to server:", e);
    }
  };

  // Load initial assets from server
  React.useEffect(() => {
    const fetchInitialAssets = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (serverAssets && !serverAssets.default) {
            const serverAssetsStr = JSON.stringify(serverAssets);
            lastServerSyncedAssetsRef.current = serverAssetsStr;
            setAssets(serverAssets);
          }
        }
      } catch (e) {
        console.error("Failed to fetch assets from server:", e);
      }
    };
  ...
  // Set up Server-Sent Events (SSE) synchronization stream
  React.useEffect(() => {
    const eventSource = new EventSource('/api/assets/sync');
    
    eventSource.onmessage = (event) => {
      try {
        const payload = JSON.parse(event.data);
        if (payload.type === 'assets-updated') {
          const nextStr = JSON.stringify(payload.assets);
          
          // Store this in ref immediately to skip redundant post in our local useEffect
          lastServerSyncedAssetsRef.current = nextStr;
  ...
  React.useEffect(() => {
    ...
    // Check if the current client state differs from the last server-synced representation
    const currentStr = JSON.stringify(assets);
    if (currentStr && currentStr !== lastServerSyncedAssetsRef.current) {
      persistAssetsToServer(assets);
    }
  }, [assets]);
